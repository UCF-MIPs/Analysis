...
def plot_total_edges_networks(graph_dict, data_name):
    result_dict = {}
    ...
    for threshold in unique_thresholds:
        ...
        total_edges = []  # Changed this line
        colors = []
        # Collect data for plotting
        for edge_type in all_edge_types:
            threshold_dict = result_dict.get(edge_type, {})
            metrics = threshold_dict.get(threshold, {'total_edges': 0})  # Changed this line
            total_edges.append(metrics['total_edges'])  # Changed this line
            colors.append(color_dict[edge_type])

        # Plot the total number of nodes and edges per edge type
        ax.bar(range(len(all_edge_types)), total_edges, bar_width, color=colors)  # Changed this line

        ax.set_yscale('log')
        plt.xticks(rotation = 45)
        ax.set_xlabel('Edge Types')
        ax.set_ylabel('Count')
        #ax.set_title(f'Total Edges at Threshold: {threshold}')  # Changed this line
        ax.grid(True)
        ...
        plt.savefig(f'./totals/thresh_{threshold}_total_edges.png')  # Changed this line
